...
class CompanyMessagesController extends Controller
{
    ...
    public function createMeeting(Request $request)
    {
        ...
        $meeting_id = uniqid();
        $meeting_url = "https://meet.jobaaty.com/" . $meeting_id;
    
        $message = new CompanyMessage();
        $message->company_id = Auth::guard('company')->user()->id;
        $message->message = "Meeting scheduled for " . $request->scheduled_at . " (" . $request->timezone . ")";
        $message->seeker_id = $seeker_id;
        $message->type = 'meeting';
        $message->meeting_topic = $request->meeting_topic;
        $message->scheduled_at = $request->scheduled_at;
        $message->meeting_url = $meeting_url; // احفظ الرابط في الحقل الخاص بالرابط إذا كان موجودًا
        $message->save();
